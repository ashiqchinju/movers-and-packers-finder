...
if(isset($_POST['login']))
{
	 ...
	 $user_password=$_POST['password'];
	    $select="select * from creg_tb where email='$user_email' and password='$user_password'"; 
	    $run=$conn->query($select);
		  if($run->num_rows>0){
		      while($row=$run->fetch_array()){
				 $_SESSION['email']=$user_email=$row['email'];
				 $success="Logged In Successfully...";
				 header("location:./customer/customerHome.php"); //we can also use.. echo"<script>window.location.href='welcome.php'</script>"; 
			}
	 } 
	 else{
...
<body  style="background-image: url('images/login.jpg'); background-repeat:no-repeat; background-size:100% 100vh;">
    ...
    <h1 class="mt-4 mb-3" > <small>Customer Login</small> </h1>

<ol class="breadcrumb bg-transparent">
    <li class="breadcrumb-item">
        <a href="index.php">Home</a>
    </li>
    <li class="breadcrumb-item active" id="mini">Customer Login</li>
</ol>
    
    <div class="row justify-content-center align-content-center">
    <div class="col-sm-8 col-sm-offset-2 bg-transparent alert-success" style="box-shadow:2px 2px 2px 2px gray;">
    <h4 class="text text-center alert bg-primary" style="color:white">Customer Login</h4>
